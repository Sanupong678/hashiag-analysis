...
class YahooFinanceFetcher:
    ...
    def get_stock_info(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if hist.empty:
                ...
                try:
                    ...
                    if info and info.get('regularMarketPrice'):
                        ...
                        current_volume = int(info.get('volume', 0))
                        ...
                        return {
                            ...
                            'averageVolume': int(average_volume) if average_volume else current_volume,
                            'bid': float(bid) if bid else None,
                            'ask': float(ask) if ask else None,
                            'bidSize': int(bid_size) if bid_size else 0,
                            'askSize': int(ask_size) if ask_size else 0,
                            'spread': float(spread) if spread else 0,
                            'spreadPercent': float(spread_percent) if spread_percent else 0,
            ...
            # ดึงข้อมูล bid/ask (ราคาที่ผู้ซื้อ/ขายเสนอ)
            bid = info.get('bid', current_price)  # ราคาที่ผู้ซื้อเสนอ
            ask = info.get('ask', current_price)  # ราคาที่ผู้ขายเสนอ
            bid_size = info.get('bidSize', 0)  # จำนวนหุ้นที่ผู้ซื้อเสนอ
            ask_size = info.get('askSize', 0)  # จำนวนหุ้นที่ผู้ขายเสนอ
            
            # คำนวณ spread (ความแตกต่างระหว่าง bid และ ask)
            if bid and ask and bid > 0 and ask > 0:
                spread = ask - bid
                spread_percent = (spread / current_price * 100) if current_price > 0 else 0
            else:
            ...
            # คำนวณ average volume จาก historical data (30 วัน)
            average_volume = 0
            try:
                # ดึง historical data 30 วันเพื่อคำนวณ average volume
                hist_30d = ticker.history(period="30d")
                if not hist_30d.empty and 'Volume' in hist_30d.columns:
                    volumes = hist_30d['Volume'].dropna()
                    if len(volumes) > 0:
                        average_volume = int(volumes.mean())
            except:
                # ถ้าดึงไม่ได้ ใช้ averageVolume จาก info (ถ้ามี)
                average_volume = info.get('averageVolume', current_volume)
            
            # ถ้ายังไม่มี average_volume ให้ใช้ current_volume
            if average_volume == 0:
            ...
            return {
                ...
                'changePercent': float(change_percent),
                'volume': current_volume,
                'averageVolume': int(average_volume),  # เพิ่ม averageVolume
                'bid': float(bid) if bid else None,  # ราคาที่ผู้ซื้อเสนอ
                'ask': float(ask) if ask else None,  # ราคาที่ผู้ขายเสนอ
                'bidSize': int(bid_size) if bid_size else 0,  # จำนวนหุ้นที่ผู้ซื้อเสนอ
                'askSize': int(ask_size) if ask_size else 0,  # จำนวนหุ้นที่ผู้ขายเสนอ
                'spread': float(spread) if spread else 0,  # ความแตกต่างระหว่าง bid และ ask
                'spreadPercent': float(spread_percent) if spread_percent else 0,  # Spread เป็นเปอร์เซ็นต์
                'marketCap': info.get('marketCap', 0),
                'sector': info.get('sector', 'Unknown'),
                'industry': info.get('industry', 'Unknown'),
                'fetchedAt': datetime.utcnow().isoformat()
